...
def _vehicle_module():
    ...
    if src in ("📷 Photo", "📸 Camera Snapshot"):
        ...
        if v:
            if st.button("🎬 Process Entire Video", type="primary", use_container_width=True, key="vd_process_btn"):
                tfile = tempfile.NamedTemporaryFile(delete=False, suffix=".mp4")
                tfile.write(v.read())
                tfile.close()
                ...
                all_counts = {"car": 0, "truck": 0, "bus": 0, "motorcycle": 0}
                all_colors = {}
                total_passengers = 0
                frame_detections = []
                ...
                while cap.isOpened():
                    ...
                    # Process every frame for quality output
                    results = model(frame, verbose=False, imgsz=640)[0]

                    # Accumulate analytics
                    frame_count = {"car": 0, "truck": 0, "bus": 0, "motorcycle": 0}
                    for box in results.boxes:
                        cls_id = int(box.cls[0])
                        name = results.names[cls_id]
                        conf = float(box.conf[0])
                        if name in frame_count:
                            ...
                            if name == 'truck' and (conf < 0.45 or (h_box > 0 and w_box/h_box < 0.8)):
                                continue
                            if name == 'car' and conf < 0.40:
                                continue
                            frame_count[name] += 1
                            # Color detection
                            if name in ['car', 'truck', 'bus']:
                                cx1, cy1 = x1 + int(w_box*0.3), y1 + int(h_box*0.3)
                                cx2, cy2 = x1 + int(w_box*0.7), y1 + int(h_box*0.7)
                    ...
                    for k in frame_count:
                        all_counts[k] += frame_count[k]
                    frame_detections.append(sum(frame_count.values()))

                    processed = process_analytics(frame.copy(), results, (0, 177, 64))
                    writer.write(processed)

                    frame_idx += 1
                    if frame_idx % 5 == 0 or frame_idx == total_frames:
                        pct = min(frame_idx / max(total_frames, 1), 1.0)
                        progress.progress(pct, text=f"Processing frame {frame_idx}/{total_frames}...")
                ...
                st.session_state["vd_processed_video"] = video_bytes
                st.session_state["vd_analytics"] = {
                    "counts": all_counts,
                    "colors": all_colors,
            ...
            if "vd_processed_video" in st.session_state:
                video_bytes = st.session_state["vd_processed_video"]
                analytics = st.session_state["vd_analytics"]

                st.success(f"✅ Video processed! {analytics['total_frames']} frames @ {analytics['fps']:.0f} FPS")

                # Video player with play/pause/scrub
                ...
                # Analytics dashboard
                st.divider()
                section_header("Video Analytics", "Full detection summary")

                # Metric cards
                m1, m2, m3, m4 = st.columns(4)
                m1.metric("🚗 Cars", analytics["counts"]["car"])
                m2.metric("🚛 Trucks", analytics["counts"]["truck"])
                m3.metric("🚌 Buses", analytics["counts"]["bus"])
                m4.metric("🏍️ Motorcycles", analytics["counts"]["motorcycle"])
                ...
                with col1:
                    ...
                    values = [val for val in counts.values() if val > 0]
                    if values:
                        import plotly.graph_objects as go_local
                        fig_pie = go_local.Figure(go_local.Pie(
                            labels=labels, values=values,
                            marker=dict(colors=["#3B82F6", "#EF4444", "#F59E0B", "#10B981"]),
                            textinfo="label+percent", textfont=dict(color="#FFFFFF"),
                        ))
                        fig_pie.update_layout(
                            title=dict(text="Vehicle Type Distribution", font=dict(color="#FFFFFF", size=16)),
                            paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                            font=dict(color="#E2E8F0"), height=320, margin=dict(t=40, b=20, l=20, r=20),
                            showlegend=False,
                        )
                ...
                with col2:
                    # Color distribution
                    if analytics["colors"]:
                        color_labels = list(analytics["colors"].keys())
                        color_values = list(analytics["colors"].values())
                        import plotly.graph_objects as go_local2
                        fig_bar = go_local2.Figure(go_local2.Bar(
                            x=color_labels, y=color_values,
                            marker_color="#F59E0B",
                            text=color_values, textposition="outside",
                            textfont=dict(color="#FFFFFF"),
                        ))
                        fig_bar.update_layout(
                            title=dict(text="Vehicle Color Distribution", font=dict(color="#FFFFFF", size=16)),
                            paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                            font=dict(color="#E2E8F0"), height=320, margin=dict(t=40, b=20, l=20, r=20),
                ...
                if analytics["frame_detections"]:
                    import plotly.graph_objects as go_local3
                    fig_timeline = go_local3.Figure(go_local3.Scatter(
                        x=list(range(len(analytics["frame_detections"]))),
                        y=analytics["frame_detections"],
                        mode="lines", fill="tozeroy",
                        line=dict(color="#3B82F6", width=2),
                        fillcolor="rgba(59,130,246,0.15)",
                    ))
                    fig_timeline.update_layout(
                        title=dict(text="Detections Over Time (per frame)", font=dict(color="#FFFFFF", size=16)),
                        paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                        font=dict(color="#E2E8F0"), height=280, margin=dict(t=40, b=30, l=40, r=20),
                        xaxis=dict(title="Frame", gridcolor="rgba(255,255,255,0.05)"),
                        yaxis=dict(title="Vehicles Detected", gridcolor="rgba(255,255,255,0.05)"),
                    )
