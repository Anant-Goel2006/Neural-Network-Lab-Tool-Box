...
def lstm_prediction_page():
    ...
    # -----------------------------
    # Sidebar/Side Column: Data Management
    # -----------------------------
    with side_col:
        st.markdown("""
        ...
        training_data = st.text_area("Training Sentences (one per line):", value=default_text, height=100)
        ...
        st.divider()
        section_header("LSTM Parameters", "Model Config")
        # -----------------------------
        # LSTM Parameters (Reset when vocab changes)
        # -----------------------------
        input_size = vocab_size
        hidden_size = st.number_input("Hidden Dimension Size", min_value=2, max_value=64, value=8)
            ...
            if st.button("🧠 Run LSTM Prediction", type="primary", use_container_width=True):
                ...
                with col_main:
                    ...
                    with st.spinner("🤖 Requesting sequence context analysis..."):
                        ...
                        prompt = f"Act as an expert AI tutor explaining step by step how this LSTM just predicted the word '{pred_word.upper()}'. Break down each step and explain the mathematical definitions clearly: what are the forget, input, and output gates? what is the cell state? Deliver it in an engaging, step-by-step tutorial format."
                        ai_text = get_ai_explanation(prompt)
                    
                    if ai_text:
                        render_nlp_insight(ai_text, "🤖 AI Tutor // Calculation Breakdown", "#00ffcc")
                        render_voice_button(ai_text, "lstm_pred_final")
                    else:
                        st.caption("AI Analysis unavailable. Check your AI Helper.")
    ...
    render_chatbot("LSTM Time-Series Processing")
